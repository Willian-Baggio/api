...
public class LivroController {
    ...
    @PostMapping
    @Transactional
    public ResponseEntity cadastrar(@RequestBody @Valid DadosCadastroLivros dados, UriComponentsBuilder uriBuilder) {
        var livro = new Livro(dados);
        repository.save(livro);

        var uri = uriBuilder.path("/livro/{id}").buildAndExpand(livro.getId()).toUri();
        var dto = new DadosDetalhamentoLivro(livro);

        return ResponseEntity.created(uri).body(dto);
    }

    @GetMapping
    public ResponseEntity<Page<Livro>> listar(@PageableDefault(size = 10, sort = {"titulo"}) Pageable paginacao) {
